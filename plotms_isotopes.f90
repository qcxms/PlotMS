!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  ! given chemical formula (nat atoms with OZ iat_sum())
...
subroutine isotope(nat, iat_sum, maxatm, rnd, mass, mint, nsig, no_isotopes)
  use xtb_mctc_accuracy, only: wp
  implicit none

  integer :: nat,iat_sum(*),nsig,maxatm
  integer :: mass(*)
  integer :: niso(200)
  integer :: nmass(10000)
  ...
  do i = 1, nat
     if ( iat_sum(i) > 100 ) then
        niso   (iat_sum(i))   = 1
        prob   (iat_sum(i),1) = 1.0_wp
        massiso(iat_sum(i),1) = iat_sum(i) - 100.0_wp
     endif
  enddo
 
  ! if number of isotopes == 0 <- often errors if wrong .res file is read (is (hopefully) fixed)
  do i=1,nat
     if(niso(iat_sum(i)) == 0) stop 'internal isotope error 2'
  enddo
